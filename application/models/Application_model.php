...
class Application_model extends CI_Model
{
    ...
    {
        // SS-PHASE1-PATCH: application_model — URL-host based tenant resolution
    ...
    // SS-PHASE1-PATCH: application_model — helper
    /**
     * Resolve the active tenant (branch_id) from the HTTP Host header by
     * looking up the `custom_domain` table. Returns null when no mapping
     * found, the multi-tenant feature is inactive, or running under CLI.
     * Cached per request.
     */
    protected function ss_resolve_branch_from_host()
    {
        ...
        return $cached_value = ($row ? (int) $row->school_id : null);
    }
    ...
    public function getStudentListByClassSection($classID = '', $sectionID = '', $branchID = '', $deactivate = false, $rollOrder = false, $is_alumni = true, $any_session = false)
    {
        // $any_session = true skips the `e.session_id = get_session_id()`
        // filter. Used as a fallback in /student/view so students enrolled
        // under a different academic year (e.g. when the topbar academic-year
        // switcher is out of sync with the saved enroll row) still show up
        // instead of the list silently appearing empty.
        ...
        if (!$any_session) {
            $sql .= " AND `e`.`session_id` = " . $this->db->escape(get_session_id());
        }
        if ($sectionID != 'all') {
