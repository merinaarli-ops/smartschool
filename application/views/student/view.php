...
<?php
// Detect whether this branch has demo students seeded by the
// NCTB seeder, so we can offer an admin a one-click cleanup.
$demoBranchId = (int)$this->application_model->get_branch_id();
$demoStudentCount = 0;
if ($demoBranchId > 0 && get_permission('student', 'is_delete')) {
    $demoStudentCount = (int)$this->db
        ->like('register_no', 'DEMO-' . $demoBranchId . '-', 'after')
        ->count_all_results('student');
}
if ($demoStudentCount > 0): ?>
<div class="row">
<div class="col-md-12">
<div class="alert alert-warning d-flex justify-content-between align-items-center" style="display:flex;justify-content:space-between;align-items:center;">
  <span>
    <i class="fas fa-flask"></i>
    <strong>Demo data detected:</strong>
    <?php echo $demoStudentCount; ?> demo student<?php echo $demoStudentCount === 1 ? '' : 's'; ?> with sample marks are seeded on this branch (Class 1 / first section).
    Useful for trying out marks entry, marksheet and ranking flows. You can remove them when no longer needed.
  </span>
  <a href="<?php echo base_url('student/demo_clear'); ?>"
     onclick="return confirm('Remove all demo students, their parents, enrolments and marks from this branch? This cannot be undone.');"
     class="btn btn-danger btn-sm">
    <i class="fas fa-trash"></i> Remove all demo data
  </a>
</div>
</div>
</div>
...
<div class="row">
	<div class="col-md-12">
		...
		<?php if (!empty($fallback_any_session)): ?>
		<div class="alert alert-info">
			<i class="fas fa-info-circle"></i>
			No students were enrolled in this class/section under the currently-active academic year, so showing students from <strong>all academic years</strong> for the same class/section instead. Switch the academic year selector in the top bar to filter strictly.
		</div>
		<?php endif; ?>
