...
/**
 * Landing variant C — "Dark Mode / Night".
 *
 * Near-black background, white text, brand-colour accents.  Sleek and modern,
 * suitable for tech-forward schools or after-dark campaigns.  Reads exactly
 * the same fields off $s as variants A and B, plus the tiered packages array
...
<meta name="description" content="Free school management software for Bangladesh. Dark, fast, modern — every module unlocked.">
<meta name="theme-color" content="#0a0d12">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700&family=Hind+Siliguri:wght@400;500;600;700&display=swap">
<style>
:root{
  ...
  --ink-soft:#c6cbd4;
  --muted:#8b94a3;
  --glow: 0 0 0 1px rgba(255,255,255,.04), 0 24px 60px -28px color-mix(in srgb, var(--brand) 65%, transparent);
}
*{box-sizing:border-box}
html{scroll-behavior:smooth}
body{font-family:'Space Grotesk','Hind Siliguri',system-ui,-apple-system,sans-serif;color:var(--ink);background:var(--bg);line-height:1.6;-webkit-font-smoothing:antialiased;margin:0}
.bn{font-family:'Hind Siliguri','Noto Sans Bengali',sans-serif}
.container{max-width:1140px;margin:0 auto;padding:0 24px}
a{color:var(--ink);text-decoration:none}
a:hover{color:var(--brand)}
h1,h2,h3,h4,h5{font-weight:700;letter-spacing:-.02em;color:var(--ink);margin:0}
p{color:var(--ink-soft);margin:0}

/* Background ambience */
body::before{content:'';position:fixed;inset:0;z-index:-1;pointer-events:none;
  background:
    radial-gradient(900px 500px at 80% -10%, color-mix(in srgb, var(--brand) 22%, transparent), transparent 60%),
    radial-gradient(700px 400px at 5% 10%, color-mix(in srgb, var(--brand) 10%, transparent), transparent 60%),
    var(--bg)}

/* Nav */
.nav{position:sticky;top:0;background:rgba(10,13,18,.7);backdrop-filter:saturate(180%) blur(14px);-webkit-backdrop-filter:saturate(180%) blur(14px);border-bottom:1px solid var(--line);z-index:100}
.nav .row{display:flex;align-items:center;justify-content:space-between;padding:18px 0}
.nav .brand{font-weight:700;font-size:1.1rem;color:var(--ink);letter-spacing:-.02em}
.nav .brand .dot{color:var(--brand)}
.nav .links{display:flex;gap:28px;align-items:center;font-size:.93rem;color:var(--ink-soft)}
.nav .links a{color:var(--ink-soft);font-weight:500}
.nav .links a:hover{color:var(--ink)}

.btn{display:inline-flex;align-items:center;gap:8px;border:1px solid transparent;border-radius:8px;padding:10px 18px;font-weight:600;font-size:.93rem;cursor:pointer;transition:transform .15s,box-shadow .15s,background .15s,color .15s}
.btn-primary{background:var(--brand);color:#fff;border-color:var(--brand);box-shadow:var(--glow)}
.btn-primary:hover{filter:brightness(1.08);transform:translateY(-1px);color:#fff}
.btn-ghost{background:transparent;color:var(--ink);border-color:var(--line-strong)}
.btn-ghost:hover{background:rgba(255,255,255,.04);color:var(--ink)}
.btn-link{background:transparent;border:none;color:var(--ink-soft);font-weight:600;padding:10px 0;border-bottom:1px solid var(--line-strong);border-radius:0}
.btn-link:hover{color:var(--brand);border-color:var(--brand)}
.btn-lg{padding:14px 24px;font-size:1rem}

/* Hero */
.hero{padding:120px 0 90px}
.hero .eyebrow{display:inline-flex;align-items:center;gap:10px;font-size:.85rem;color:var(--ink-soft);font-weight:500;margin-bottom:24px;padding:6px 14px;border:1px solid var(--line-strong);border-radius:999px;background:rgba(255,255,255,.03)}
.hero .eyebrow .dot{width:8px;height:8px;border-radius:50%;background:var(--brand);box-shadow:0 0 0 4px color-mix(in srgb, var(--brand) 24%, transparent)}
.hero h1{font-size:clamp(2.5rem,5.6vw,4.4rem);line-height:1.05;margin-bottom:24px;max-width:18ch;letter-spacing:-.035em}
.hero h1 .accent{background:linear-gradient(135deg,var(--brand),#fff);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent}
.hero .bn-tag{font-size:1.05rem;color:var(--muted);margin-bottom:24px;max-width:560px}
.hero .lead{font-size:1.1rem;color:var(--ink-soft);margin-bottom:36px;max-width:620px;line-height:1.6}
...
.hero .stat .num{font-size:1.85rem;font-weight:700;letter-spacing:-.03em;color:var(--ink);line-height:1.1}
.hero .stat .lbl{font-size:.78rem;color:var(--muted);text-transform:uppercase;letter-spacing:.1em;margin-top:4px}
...
.section h2{font-size:clamp(1.8rem,3.6vw,2.6rem);line-height:1.15;letter-spacing:-.025em;margin-bottom:14px}
...
.feat-grid .item{padding:30px 26px;background:var(--bg-2);border:1px solid var(--line);border-radius:14px;transition:transform .2s,border-color .2s,background .2s}
.feat-grid .item:hover{transform:translateY(-3px);border-color:var(--line-strong);background:var(--bg-3)}
.feat-grid .item .num{font-size:.78rem;font-weight:700;color:var(--brand);letter-spacing:.12em;margin-bottom:14px;display:block}
.feat-grid .item h5{font-size:1.1rem;margin-bottom:10px;letter-spacing:-.02em}
.feat-grid .item p{font-size:.94rem;color:var(--ink-soft);line-height:1.6}

/* Pricing — free callout */
.price-block{display:grid;grid-template-columns:1.1fr 1fr;gap:60px;align-items:center;background:var(--bg-2);border:1px solid var(--line);border-radius:20px;padding:48px}
.price-block .badge{display:inline-block;font-size:.78rem;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--brand);background:color-mix(in srgb, var(--brand) 14%, transparent);padding:6px 12px;border-radius:999px;margin-bottom:20px}
.price-block h2{font-size:clamp(2rem,4vw,2.8rem);line-height:1.1;margin-bottom:18px;letter-spacing:-.025em}
.price-block .big-price{font-size:clamp(4.5rem,11vw,7rem);font-weight:700;letter-spacing:-.05em;line-height:1;color:var(--ink);margin-bottom:14px}
...
.price-block .perks{list-style:none;padding:0;margin:0 0 24px;display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px 24px}
...
.price-block .future-note{margin-top:24px;padding:16px 18px;background:var(--bg-3);border-left:3px solid var(--brand);border-radius:0 10px 10px 0;font-size:.92rem;color:var(--ink-soft);line-height:1.55}
...
.price-card{background:var(--bg-2);border:1px solid var(--line);border-radius:16px;padding:32px 26px;display:flex;flex-direction:column;transition:transform .15s,border-color .15s,box-shadow .15s}
.price-card:hover{transform:translateY(-3px);border-color:var(--line-strong);box-shadow:0 18px 40px rgba(0,0,0,.35)}
.price-card.featured{border-color:var(--brand);background:linear-gradient(180deg, color-mix(in srgb, var(--brand) 10%, var(--bg-2)) 0%, var(--bg-2) 100%);box-shadow:var(--glow)}
.price-card .badge-pop{display:inline-block;background:var(--brand);color:#fff;padding:4px 10px;border-radius:999px;font-size:.72rem;font-weight:700;letter-spacing:.04em;text-transform:uppercase;margin-bottom:12px;align-self:flex-start}
.price-card .plan-name{font-weight:700;font-size:1.1rem;margin-bottom:12px}
.price-card .price{font-size:2.2rem;font-weight:700;letter-spacing:-.03em;margin-bottom:4px;color:var(--ink)}
...
/* Testimonials */
.t-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:24px}
.t-grid .t{padding:32px 28px;background:var(--bg-2);border:1px solid var(--line);border-radius:16px}
...
/* Final CTA */
.cta-strip{padding:96px 0;text-align:center;border-top:1px solid var(--line);position:relative;overflow:hidden}
.cta-strip::before{content:'';position:absolute;inset:0;z-index:-1;background:radial-gradient(700px 400px at 50% 100%, color-mix(in srgb, var(--brand) 28%, transparent), transparent 60%)}
.cta-strip h2{font-size:clamp(2rem,4.4vw,3rem);line-height:1.1;letter-spacing:-.03em;margin-bottom:18px;max-width:18ch;margin-left:auto;margin-right:auto}
.cta-strip p{color:var(--ink-soft);font-size:1.05rem;margin-bottom:30px;max-width:520px;margin-left:auto;margin-right:auto}
...
footer .brand{display:inline-block;font-weight:700;font-size:1.05rem;letter-spacing:-.02em;color:var(--ink);margin-bottom:10px}
...
<section class="hero">
  <div class="container">
    <div class="reveal d1">
      ...
      <h1><?=htmlspecialchars($h1)?> <span class="accent">Free.</span></h1>
...
<section class="section" id="features">
  <div class="container">
    <div class="head reveal d1">
      ...
      <h2>Every module unlocked. No paywalls, no upsells.</h2>
      <p class="lead-p">Built for the way Bangladeshi schools actually operate — admissions to accounting in one place.</p>
    </div>
    <div class="feat-grid">
