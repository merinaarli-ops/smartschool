...
<?php
// ============================================================
// Auto-setup Class, Subject & Exam — admin-only card
// (Reads `branch.institute_type` + `branch.institute_subtype`
// and runs the NCTB seeder filtered to the matching subset.)
// ============================================================
if (is_admin_loggedin() || is_superadmin_loggedin()) {
    $autoBranchId = (int)$this->application_model->get_branch_id();
    $autoClassCount = 0;
    $autoInstType = '';
    $autoInstSubtype = '';
    if ($autoBranchId > 0) {
        $autoClassCount = (int)$this->db->where('branch_id', $autoBranchId)->count_all_results('class');
        if ($this->db->field_exists('institute_type', 'branch')) {
            $br = $this->db->select('institute_type, institute_subtype')
                           ->where('id', $autoBranchId)
                           ->get('branch')->row_array();
            $autoInstType    = (string)($br['institute_type']    ?? '');
            $autoInstSubtype = (string)($br['institute_subtype'] ?? '');
        }
    }
    ...
    $autoTypeLabel    = $autoTypeLabels[$autoInstType] ?? '';
    $autoSubtypeLabel = $autoSubtypeLabels[$autoInstSubtype] ?? '';
?>
<div class="row" style="margin-bottom:14px;">
    <div class="col-md-12">
        <div style="background: linear-gradient(135deg, #2563eb 0%, #16a34a 100%); border-radius: 12px; padding: 18px 22px; color:#fff; display:flex; align-items:center; gap:18px; flex-wrap:wrap; box-shadow: 0 8px 24px -8px rgba(37, 99, 235, .35);">
            <div style="flex:0 0 auto; font-size:42px; line-height:1;">
                <i class="fas fa-magic"></i>
            </div>
            <div style="flex:1 1 auto; min-width:240px;">
                <div style="font-size:18px; font-weight:700; line-height:1.3;">
                    স্বয়ংক্রিয় সেটআপ — ক্লাস, বিষয় ও পরীক্ষা
                    <small style="display:block; font-size:13px; font-weight:500; opacity:.92; margin-top:2px;">
                        Auto-setup classes, subjects &amp; exams (one click · NCTB Bangla)
                    </small>
                </div>
                <div style="font-size:13px; opacity:.92; margin-top:6px;">
                    <?php if ($autoTypeLabel !== ''): ?>
                        <strong>Institute type:</strong> <?= html_escape($autoTypeLabel); ?>
                        <?php if ($autoSubtypeLabel !== ''): ?>
                            · <strong>Sub-type:</strong> <?= html_escape($autoSubtypeLabel); ?>
                        <?php endif; ?>
                        ·
                    <?php endif; ?>
                    <?php if ($autoClassCount > 0): ?>
                        <?= (int)$autoClassCount; ?> ক্লাস ইতিমধ্যে রয়েছে — পুনরায় চালালে শুধু অনুপস্থিত আইটেম যোগ হবে (idempotent).
                    <?php else: ?>
                        কোনো ক্লাস এখনো যোগ হয়নি — এক ক্লিকে সব সেটআপ করুন।
                    <?php endif; ?>
                </div>
            </div>
            <div style="flex:0 0 auto;">
                <?= form_open(base_url('subject/auto_setup'), [
                    'style'    => 'margin:0;',
                    'onsubmit' => "return confirm('Run auto-setup now? This will add missing classes, subjects, exam terms and starter exams for this branch (existing rows are skipped).')",
                ]); ?>
                    <button type="submit" class="btn btn-lg" style="background:#fff; color:#1e3a8a; font-weight:700; padding:12px 22px; border-radius:8px; box-shadow:0 4px 12px -2px rgba(0,0,0,.25);">
                        <i class="fas fa-bolt"></i>
                        <?php if ($autoClassCount > 0): ?>
                            পুনরায় চালান · Re-run setup
                        <?php else: ?>
                            এক ক্লিকে সেটআপ · Run auto-setup
                        <?php endif; ?>
                    </button>
                <?= form_close(); ?>
            </div>
