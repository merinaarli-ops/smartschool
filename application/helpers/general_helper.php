...
{
    // Guard against null / empty input. The `languages.word` column is
    // NOT NULL, so calling translate(null) (e.g. from a profile view where
    // an optional student field is missing) was throwing a SQL error.
    if ($word === null || $word === '') {
        return '';
    }
    ...
    $sql = "SELECT `english`,`" . $set_lang . "` FROM `languages` WHERE `word` = " . $CI->db->escape($word);
...
{
    // SS-FIX: self-heal stale session_id.
    //
    // The active academic session id is resolved from the user's
    // CI session userdata (`set_session_id`) with a fallback to
    // `global_settings.session_id`. Both are written once and then
    // never reconciled with the `schoolyear` table — so if a super
    // admin later deletes the schoolyear row that those values
    // point at, every downstream INSERT that FKs to schoolyear
    // (notably `exam.session_id` and `subject_assign.session_id`,
    // both of which are FOREIGN KEY ... REFERENCES schoolyear(id))
    // silently fails on a 1452 FK constraint, while the controller
    // still calls set_alert('success', ...). The user sees a
    // "saved successfully" banner but the new row never appears.
    //
    // We resolve the stored id once per request, verify it exists
    // in `schoolyear`, and — if it doesn't — fall back to the
    // newest existing schoolyear row, persisting the heal back to
    // `global_settings` and the user's CI session so the rest of
    // the request (and subsequent ones) see a consistent value.
    ...
    // IMPORTANT: this function is frequently called *inline* while
    // a caller is still mid-way through building a query builder
    // chain (e.g. `$this->db->where(...)->where('session_id',
    // get_session_id())->...`). Using `$CI->db` query builder
    // methods (where/get/count_all_results/update) here would
    // corrupt the caller's pending state, so the heal must use
    // raw `$CI->db->query(...)` calls which don't touch query
    // builder state.
    try {
        if (!empty($CI->db) && $CI->db->table_exists('schoolyear')) {
            $exists = false;
            if ($session_id > 0) {
                $row = $CI->db->query(
                    'SELECT 1 AS c FROM `schoolyear` WHERE `id` = ? LIMIT 1',
                    array($session_id)
                )->row();
                $exists = !empty($row);
            }
            if (!$exists) {
                ...
                if (!empty($row) && !empty($row->id)) {
                    ...
                    // Persist the heal so the next request also
                    // resolves to a valid id without a re-query.
                    try {
                        $CI->db->query(
                            'UPDATE `global_settings` SET `session_id` = ? WHERE `id` = 1',
                            array($session_id)
                        );
                    } catch (\Throwable $e) {
                        log_message('error', 'get_session_id heal global_settings: ' . $e->getMessage());
                    }
                    if ($CI->session->has_userdata('set_session_id')) {
        ...
        log_message('error', 'get_session_id self-heal failed: ' . $e->getMessage());
    }
