            ...
            if ($this->form_validation->run() !== false) {
                ...
                if (empty($sectionID)) {
                    if (get_permission('section', 'is_add')) {
                        ...
                        // Auto-mirror every NCTB subject onto the new section so
                        // the freshly-added section inherits the existing class →
                        // subject grid without the admin clicking class_assign.
                        try {
                            $this->load->library('nctb_subject_seeder');
                            $this->nctb_subject_seeder->mirrorSubjectsOntoNewSection(
                                (int)$arraySection['branch_id'],
                                $newSectionId
                            );
                        } catch (Exception $e) {
                            log_message('error', 'NCTB mirror to new section failed: ' . $e->getMessage());
                        }
                    }
