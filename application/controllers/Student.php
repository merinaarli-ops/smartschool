...
class Student extends Admin_Controller
{
    ...
    public function quick_add()
    {
        ...
        // Superadmins must pick a branch first (same UX as create_admission).
        // For non-superadmins, branch is implicit from the logged-in user.
        $branchID = $this->application_model->get_branch_id();
    ...
    /**
     * AJAX endpoint for the Quick Admission form. Mirrors `Student::save()`
     * (the create_admission endpoint) by responding with JSON instead of a
     * server-side redirect — the page stays put on `student/quick_add`, a
     * toast appears, and the form is reset client-side. This is required
     * because on multi-tenant subdomains a server-side redirect to either
     * `/student/profile/{id}` or `/student/view` runs into a
     * `get_session_id()` mismatch and either 404s or shows an empty list.
     */
    public function quick_save()
    {
        ...
        if (empty($branchID)) {
            echo json_encode(array(
                'status' => 'fail',
                'error'  => array('branch_id' => translate('branch') . ' ' . translate('field_is_required')),
            ));
            return;
        }
        ...
        // Always enroll into the user's *currently active* session so the new
        // student shows up in /student/view straight away. (Letting the user
        // pick a different year here was the source of the "saved students
        // don't appear in the list" bug.)
        $sessionID = (int) get_session_id();
        if ($sessionID <= 0) {
            $sessionID = (int) get_global_setting('session_id');
        }
        ...
        // Wrap the three inserts in a transaction so a failure on enroll
        // (FK / NOT NULL / unique violations) doesn't leave behind orphan
        // student + login_credential rows.
        $this->db->trans_start();
        ...
        if ($this->db->trans_status() === false) {
            echo json_encode(array(
                'status'  => 'fail',
                'error'   => array('full_name' => translate('information_could_not_be_saved')),
    ...
    public function view()
    {
        ...
        if (isset($_POST['search'])) {
            ...
            // If the strict (current-session) lookup is empty, retry across
            // all academic sessions for the same branch+class+section. This
            // makes students created via create_admission (or quick_add) into
            // a *different* year still visible from /student/view — they were
            // disappearing on tenant subdomains because the topbar
            // academic-year switcher kept `get_session_id()` out of sync
            // with `enroll.session_id`.
            $this->data['fallback_any_session'] = false;
            if (empty($students)) {
                $fallback = $this->application_model->getStudentListByClassSection(
                    $classID, $sectionID, $branchID, false, true, true, true
                );
                if (!empty($fallback)) {
                    $students = $fallback;
                    $this->data['fallback_any_session'] = true;
                }
            }
    ...
    /**
     * Remove every demo row from the current branch in one click.
     * Touches only rows whose student.register_no starts with
     * 'DEMO-<branch_id>-' (and the parent rows their mobileno marks
     * as demo) so non-demo data is never affected.
     */
    public function demo_clear()
    {
        ...
        if ($branchId <= 0) {
            set_alert('error', 'No branch context — cannot remove demo data.');
            redirect(base_url('student/view'));
        }
        $result = ['marks' => 0, 'enrolls' => 0, 'students' => 0, 'parents' => 0];
        try {
            $this->load->library('nctb_subject_seeder');
            $result = $this->nctb_subject_seeder->purgeDemoForBranch($branchId);
        } catch (Exception $e) {
            log_message('error', 'Student::demo_clear: ' . $e->getMessage());
            set_alert('error', 'Failed to remove demo data: ' . $e->getMessage());
            redirect(base_url('student/view'));
        }
        if (array_sum($result) > 0) {
            set_alert('success', sprintf(
                'Removed %d demo student%s, %d enrolment%s, %d mark row%s and %d parent%s.',
                $result['students'], $result['students'] === 1 ? '' : 's',
                $result['enrolls'],  $result['enrolls']  === 1 ? '' : 's',
                $result['marks'],    $result['marks']    === 1 ? '' : 's',
                $result['parents'],  $result['parents']  === 1 ? '' : 's'
            ));
        } else {
            set_alert('info', 'No demo data found on this branch — nothing to remove.');
        }
        redirect(base_url('student/view'));
    }
