...
class Section extends Admin_Controller
{
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $arrayWellcome = array(
					...
					'principal_short_message' => $this->input->post('principal_short_message'),
					'principal_long_message' => $this->input->post('principal_long_message'),
					'sovapoti_short_message' => $this->input->post('sovapoti_short_message'),
					'sovapoti_long_message' => $this->input->post('sovapoti_long_message'),
    ...
    public function home_teachers()
    {
        if ($_POST) {
            ...
            $this->form_validation->set_rules('photo', translate('photo'), 'callback_photoHandleUpload[photo]');
			$this->form_validation->set_rules('principalphoto', translate('photo'), 'callback_photoHandleUpload[principalphoto]');
            if (isset($_FILES["photo"]) && empty($_FILES["photo"]['name']) && empty($_POST['old_photo'])) {
                $this->form_validation->set_rules('photo', translate('photo'), 'required');
            }
            if ($this->form_validation->run() == true) {
