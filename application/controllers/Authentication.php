            ...
            if ($this->form_validation->run() !== false) {
                ...
                if ($login_credential) {
                    if ($login_credential->active) {
                        ...
                        // SS-PHASE1-PATCH: authentication — cross-tenant login guard
                        // If the request URL is mapped to a specific tenant
                        // (via custom_domain table), the logging-in user MUST
                        // belong to that tenant. Superadmins (role 1) are
                        // allowed to sign in from anywhere.
                        if ($login_credential->role != 1
                            && $this->db->table_exists('custom_domain')
                            && !empty($_SERVER['HTTP_HOST'])) {
                            $ss_host = strtolower($_SERVER['HTTP_HOST']);
                            $ss_host = preg_replace('/:\d+$/', '', $ss_host);
                            if (substr($ss_host, 0, 4) === 'www.') {
                                $ss_host = substr($ss_host, 4);
                            }
                            $ss_row = $this->db
                                ->select('school_id')
                                ->where('url', $ss_host)
                                ...
                                ->get('custom_domain')
                                ->row();
                            if ($ss_row && (int) $ss_row->school_id !== (int) $getUser['branch_id']) {
                                set_alert('error', 'You are not authorised to log in to this school.');
                                redirect(base_url('authentication'));
                                exit();
                            }
                        }
