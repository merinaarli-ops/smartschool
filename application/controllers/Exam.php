...
class Exam extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        // Self-healing auto-backfill: if this branch has no exams yet
        // (brand-new tenant OR provisioned before the seeder shipped),
        // silently run the NCTB seeder so /exam lands on a fully-wired
        // page on first visit. Best-effort — failures are logged
        // and the page still renders normally.
        if (!$_POST && get_permission('exam', 'is_add') && !is_superadmin_loggedin()) {
            $branchId = (int)$this->application_model->get_branch_id();
            if ($branchId > 0) {
                $hasExams = (int)$this->db->where('branch_id', $branchId)
                                          ->where('session_id', get_session_id())
                                          ->count_all_results('exam');
                if ($hasExams === 0) {
                    try {
                        $this->load->library('nctb_subject_seeder');
                        $this->nctb_subject_seeder->seedEverythingForBranch($branchId);
                    } catch (Exception $e) {
                        log_message('error', 'NCTB exam auto-backfill failed for branch ' . $branchId . ': ' . $e->getMessage());
                    }
                }
            }
        }
    ...
    public function mark_entry()
    {
        ...
        // max_mark_<i> values come back empty. Falls back to the
        // branch_id field on the filter form when superadmin
        // hasn't picked a tenant yet (get_branch_id() is NULL).
